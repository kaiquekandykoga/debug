...
module DEBUGGER__
  module TestUtils
    ...
    def setup_remote_debuggee(cmd)
      ...
      c = remote_info.r.read(1) # wait for the remote server to boot up
      assert_equal 'D', c
      remote_info
    end
    ...
    def setup_tcpip_remote_debuggeee
      ...
      nil while /wait for debugger connection/ =~ (line = remote_info.r.gets) # skip this line
      if /\(.+:(\d+)\)/ =~ line
        remote_info.port = $1
      else
        raise "can not get port number: #{line.dump}"
      end
      remote_info
    end
